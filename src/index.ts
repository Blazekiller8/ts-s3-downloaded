  ...
  transports: [
    new winston.transports.Console({
      format: winston.format.combine(
        ...
        ecsFormat(),
        format.printf(info => formatPrint(info))
      ),
    }),
...
async function downloadFilesParallel(): Promise<void> {
  // List objects in the specified S3 bucket and folder
  const listCommandInput = {
    Bucket: S3_BUCKET_NAME,
    EncodingType: 'url',
    Prefix: S3_FOLDER_NAME,
    MaxKeys: 1000, // Limit the number of objects per page to 1000
...
async function downloadFile(key: string, fullFilePath: string): Promise<void> {
  ...
  try {
    ...
    if (objectBuffer.Body) {
      await promisify(pipeline)(objectBuffer.Body as Readable, fileStream);
    }
    ...
    if (getCommandError.$metadata) {
      const {
        $metadata: {requestId, cfId, extendedRequestId, httpStatusCode},
